    ...
    public class CandidatoRepository : ICandidatoRepository
    {
        ...
        public bool AtualizarCandidato(int idUsuario, Candidato CandidatoAtualizado)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    Candidato CandidatoBuscado = ctx.Candidatos.FirstOrDefault(c => c.IdUsuario == idUsuario);
        ...
        public bool RevogarInscricao(int idInscricao, int idCandidato)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    Inscricao inscricaoBuscada = ctx.Inscricaos.FirstOrDefault(i => i.IdInscricao == idInscricao && i.IdCandidato == idCandidato);
        ...
        public bool VerificarSeInscricaoExiste(int idVaga, int idCandidato)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    Inscricao InscricaoBuscada = ctx.Inscricaos.FirstOrDefault(e => e.IdCandidato == idCandidato && e.IdVaga == idVaga);
                    if (InscricaoBuscada != null)
                        return true;
        ...
        public List<ListarVagasViewModel> ListarVagasArea(int idCurso)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    Curso c = ctx.Cursos.Select(u => new Curso { IdCurso = u.IdCurso, IdArea = u.IdArea }).FirstOrDefault(u => u.IdCurso == idCurso);
                    List<ListarVagasViewModel> listvagas = new List<ListarVagasViewModel>();
                    // Declara a SqlConnection passando a string de conexão
                    using (SqlConnection con = new SqlConnection(stringConexao))
                    {
