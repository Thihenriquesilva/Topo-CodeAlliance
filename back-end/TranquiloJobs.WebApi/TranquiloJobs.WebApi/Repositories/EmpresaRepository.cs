    ...
    public class EmpresaRepository : UsuarioRepository, IEmpresaRepository
    {
        ...
        public bool AdicionarVaga(Vaga vaga)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    ...
                    ctx.Add(vaga);
                    ctx.SaveChanges();
                    var VagaNova = ctx.Vagas.FirstOrDefault(v => v == vaga);
                    AdicionarTecnologiaNaVaga(new VagaTecnologium { IdVaga = VagaNova.IdVaga, IdTecnologia = 1 });
                    return true;
                }
                catch (Exception)
                {
        ...
        public bool AtualizarVaga(int idVaga, AtualizarVagaViewModel vaga)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    Vaga vagaBuscada = ctx.Vagas.Find(idVaga);
                    if (vagaBuscada == null)
                        return false;
        ...
        public bool DeletarVaga(int idVaga)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    ...
                    List<Inscricao> BuscarInscricoes = ctx.Inscricaos.Where(u => u.IdVaga == vagaBuscada.IdVaga).ToList();
                    for (int i = 0; i < BuscarInscricoes.Count; i++)
                    {
                        ctx.Remove(BuscarInscricoes[i]);
                        ctx.SaveChanges();
                    }
                    List<VagaTecnologium> VagaTecnologia = ctx.VagaTecnologia.Where(v => v.IdVaga == vagaBuscada.IdVaga).ToList();
        ...
        public bool VerificarSeTecnologiaFoiAdicionada(int idTecnologia, int idVaga)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    VagaTecnologium vaga = ctx.VagaTecnologia.FirstOrDefault(v => v.IdTecnologia == idTecnologia && v.IdVaga == idVaga);
                    if (vaga != null)
                        return true;
        ...
        public void ExpirarVaga()
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    List<Vaga> VagasExpiradas = ctx.Vagas.Where(v => v.DataExpiracao >= DateTime.Now).ToList();
        ...
        public bool AdicionarTecnologiaNaVaga(VagaTecnologium vagaTecnologia)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    VagaTecnologium vaga = ctx.VagaTecnologia.FirstOrDefault(u => u.IdVaga == vagaTecnologia.IdVaga && u.IdTecnologia == 1);
                    if (vaga == null)
                    {
        ...
        public bool RemoverTecnologiaDaVaga(VagaTecnologium vaga)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
        ...
        public bool VerificarSeaVagaPertenceaEmpresa(int idEmpresa, int idVaga)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    Vaga vagaBuscada = ctx.Vagas.FirstOrDefault(a => a.IdEmpresa == idEmpresa && a.IdVaga == idVaga);
                    if (vagaBuscada == null)
                        return true;
        ...
        public Empresa BuscarEmpresaPorIdUsuario(int idUsuario)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    return ctx.Empresas.Select(u => new Empresa
                    {
                        IdEmpresa = u.IdEmpresa,
                        IdUsuario = u.IdUsuario,
                        NomeReponsavel = u.NomeReponsavel,
                        Cnpj = u.Cnpj,
        ...
        public List<TipoRegimePresencial> ListarTipoPresenca()
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    return ctx.TipoRegimePresencials.ToList();
                }
                catch (Exception)
                {
                    return null;
        ...
        public List<Candidato> ListarCandidatosEstagiandoNaEmpresa(int idEmpresa)
        {
            using (JobsContext ctx = new JobsContext())
            {
                try
                {
                    ...
                    var Estagios = ctx.Estagios.Where(u => u.IdEmpresa == idEmpresa).ToList();
                    for (int i = 0; i < Estagios.Count; i++)
                    {
                        var c = ctx.Candidatos.Select(u => new Candidato { IdCandidato = u.IdCandidato, Cpf = u.Cpf, Telefone = u.Telefone, NomeCompleto = u.NomeCompleto, IdUsuarioNavigation = new Usuario { CaminhoImagem = u.IdUsuarioNavigation.CaminhoImagem } }).FirstOrDefault(u => u.IdCandidato == Estagios[i].IdCandidato);
                        cList.Add(c);
                    }
                    return cList;
                }
